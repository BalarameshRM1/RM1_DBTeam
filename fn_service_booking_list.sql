...
$BODY$;
------------------------------------------------------------------------
--Remodified by Anand A on 06-11-2025.
--To retrieve the latest records, sort by created_date in descending order.
--DROP FUNCTION IF EXISTS fn_service_booking_list;
CREATE OR REPLACE FUNCTION public.fn_service_booking_list(
    tickt_id bigint DEFAULT NULL::bigint,
    custid bigint DEFAULT NULL::bigint,
...
---To include nested JSON aggregation for related services under each booking and Changed service_tracking from INNER JOIN to LEFT JOIN to include bookings without service_tracking records.
--DROP FUNCTION IF EXISTS fn_service_booking_list;
CREATE OR REPLACE FUNCTION public.fn_service_booking_list(
    tickt_id bigint DEFAULT NULL::bigint,
    custid bigint DEFAULT NULL::bigint,
	...
	p_limit  int DEFAULT 100,
    p_offset int DEFAULT 0)
    RETURNS TABLE(id bigint, booking_id character varying, slot_id bigint, slot_time character varying, hours integer, add_on_hours integer, full_name character varying, phone character varying, email character varying, address character varying, assign_to bigint, employee_name character varying, employee_email character varying, status_id bigint, status character varying,services json, total numeric, subtotal numeric, customer_requested_amount numeric, discount_amount numeric, discount_percentage numeric, discount_total numeric, created_by bigint, customer_name character varying, created_date timestamp without time zone, preferred_date date,room_sqfts varchar,with_basement boolean) 
    LANGUAGE plpgsql
    COST 100
    VOLATILE PARALLEL UNSAFE
    ROWS 1000
...
select sb.id,sb.booking_id,sb.slot_id,ms.slot_time,sb.hours,sb.add_on_hours,sb.full_name,sb.phone,sb.email,sb.address,sb.assign_to,concat_ws(' ',ur.first_name ,ur.last_name)::varchar as employee_name,ur.email as employee_email,
sb.status_id,ms2.status,COALESCE(json_agg(json_build_object('dept_id', st.dept_id,'department_name', md.department_name,
'service_id', st.service_id,'service_name', msc.service_name,'service_type_id', st.service_type_id,'service_type', mst.service_type)) FILTER (WHERE st.id IS NOT NULL),'[]'::json) AS services,
sb.total,sb.subtotal,sb.customer_requested_amount,sb.discount_amount,sb.discount_percentage,sb.discount_total,
sb.created_by,concat_ws(' ',ur2.first_name ,ur2.last_name)::varchar as customer_name,sb.created_date,sb.preferred_date,st.room_sqfts,st.with_basement
from service_booking sb 
left join service_tracking st on st.service_booking_id = sb.id and sb.is_active =true and st.is_active =true 
left join master_slots ms on ms.id = sb.slot_id and ms.is_active =true
left join master_department md on md.id = st.dept_id and md.is_active =true
left join master_service msc on msc.id = st.service_id and msc.is_active = true
left join master_service_type mst on mst.id = st.service_type_id and mst.is_active =true
...
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
WHERE (sb.id = tickt_id or -1=tickt_id)
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
    sb.discount_total, sb.created_by, ur2.first_name, ur2.last_name, 
    sb.created_date, sb.preferred_date,st.room_sqfts,st.with_basement
order by sb.created_date desc
LIMIT p_limit OFFSET p_offset;
...
RETURN QUERY
select sb.id,sb.booking_id,sb.slot_id,ms.slot_time,sb.hours,sb.add_on_hours,sb.full_name,sb.phone,sb.email,sb.address,sb.assign_to,concat_ws(' ',ur.first_name ,ur.last_name)::varchar as employee_name,ur.email as employee_email,
sb.status_id,ms2.status,
COALESCE(json_agg(json_build_object('dept_id', st.dept_id,'department_name', md.department_name,
'service_id', st.service_id,'service_name', msc.service_name,'service_type_id', st.service_type_id,'service_type', mst.service_type)) FILTER (WHERE st.id IS NOT NULL),'[]'::json) AS services,
sb.total,sb.subtotal,sb.customer_requested_amount,sb.discount_amount,sb.discount_percentage,sb.discount_total,
sb.created_by,concat_ws(' ',ur2.first_name ,ur2.last_name)::varchar as customer_name,sb.created_date,sb.preferred_date,st.room_sqfts,st.with_basement
from service_booking sb 
...
--Remodified for removing duplicate services in services json data.
--DROP FUNCTION IF EXISTS fn_service_booking_list;
CREATE OR REPLACE FUNCTION public.fn_service_booking_list(
    tickt_id bigint DEFAULT NULL::bigint,
    custid bigint DEFAULT NULL::bigint,
...
select sb.id,sb.booking_id,sb.slot_id,ms.slot_time,sb.hours,sb.add_on_hours,sb.full_name,sb.phone,sb.email,sb.address,sb.assign_to,concat_ws(' ',ur.first_name ,ur.last_name)::varchar as employee_name,ur.email as employee_email,
sb.status_id,ms2.status,COALESCE(jsonb_agg(DISTINCT jsonb_build_object('dept_id', st.dept_id,'department_name', md.department_name,'service_id', st.service_id,
'service_name', msc.service_name,'service_type_id', st.service_type_id,'service_type', mst.service_type)) 
FILTER (WHERE st.id IS NOT NULL),'[]'::jsonb)::json AS services,
sb.total,sb.subtotal,sb.customer_requested_amount,sb.discount_amount,sb.discount_percentage,sb.discount_total,
sb.created_by,concat_ws(' ',ur2.first_name ,ur2.last_name)::varchar as customer_name,sb.created_date,sb.preferred_date,st.room_sqfts,st.with_basement
from service_booking sb 
left join service_tracking st on st.service_booking_id = sb.id and st.is_active =true 
left join master_slots ms on ms.id = sb.slot_id and ms.is_active =true
left join master_department md on md.id = st.dept_id and md.is_active =true
left join master_service msc on msc.id = st.service_id and msc.is_active = true
left join master_service_type mst on mst.id = st.service_type_id and mst.is_active =true
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
WHERE (sb.id = tickt_id or -1=tickt_id)  and sb.is_active =true 
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
...
select sb.id,sb.booking_id,sb.slot_id,ms.slot_time,sb.hours,sb.add_on_hours,sb.full_name,sb.phone,sb.email,sb.address,sb.assign_to,concat_ws(' ',ur.first_name ,ur.last_name)::varchar as employee_name,ur.email as employee_email,
sb.status_id,ms2.status,
COALESCE(jsonb_agg(DISTINCT jsonb_build_object('dept_id', st.dept_id,'department_name', md.department_name,'service_id', st.service_id,
'service_name', msc.service_name,'service_type_id', st.service_type_id,'service_type', mst.service_type)) 
FILTER (WHERE st.id IS NOT NULL),'[]'::jsonb)::json AS services,
sb.total,sb.subtotal,sb.customer_requested_amount,sb.discount_amount,sb.discount_percentage,sb.discount_total,
sb.created_by,concat_ws(' ',ur2.first_name ,ur2.last_name)::varchar as customer_name,sb.created_date,sb.preferred_date,st.room_sqfts,st.with_basement
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
where (sb.created_by =custid or -1=custid) and sb.is_active =true 
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
where (sb.assign_to =empid or -1=empid)  and sb.is_active =true 
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
...
------------------------------------------------------------------------------------
--Authored by Anand A on 07-11-2025.
--Remodified for p_status_id IN parameter.
--DROP FUNCTION IF EXISTS fn_service_booking_list;
CREATE OR REPLACE FUNCTION public.fn_service_booking_list(
    tickt_id bigint DEFAULT NULL::bigint,
    custid bigint DEFAULT NULL::bigint,
    empid bigint DEFAULT NULL::bigint,
	p_status_id bigint DEFAULT -1::bigint,
	p_limit  int DEFAULT 100,
    p_offset int DEFAULT 0)
    RETURNS TABLE(id bigint, booking_id character varying, slot_id bigint, slot_time character varying, hours integer, add_on_hours integer, full_name character varying, phone character varying, email character varying, address character varying, assign_to bigint, employee_name character varying, employee_email character varying, status_id bigint, status character varying,services json, total numeric, subtotal numeric, customer_requested_amount numeric, discount_amount numeric, discount_percentage numeric, discount_total numeric, created_by bigint, customer_name character varying, created_date timestamp without time zone, preferred_date date,room_sqfts varchar,with_basement boolean) 
    LANGUAGE plpgsql
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
WHERE (sb.id = tickt_id or -1=tickt_id)  and sb.is_active =true and (sb.status_id=p_status_id or -1=p_status_id)
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
where (sb.created_by =custid or -1=custid) and sb.is_active =true and (sb.status_id=p_status_id or -1=p_status_id)
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
where (sb.assign_to =empid or -1=empid)  and sb.is_active =true and (sb.status_id=p_status_id or -1=p_status_id)
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
...
$BODY$;
------------------------------------------------------------
--Authored by Anand on 17-11-2025.
--Added for all filters in one query.
--DROP FUNCTION IF EXISTS fn_service_booking_list;
CREATE OR REPLACE FUNCTION public.fn_service_booking_list(
    tickt_id bigint DEFAULT -1::bigint,
    custid bigint DEFAULT -1::bigint,
    empid bigint DEFAULT -1::bigint,
	p_status_id bigint DEFAULT -1::bigint,
	p_limit  int DEFAULT 100,
    p_offset int DEFAULT 0)
    RETURNS TABLE(id bigint, booking_id character varying, slot_id bigint, slot_time character varying, hours integer, add_on_hours integer, full_name character varying, phone character varying, email character varying, address character varying, assign_to bigint, employee_name character varying, employee_email character varying, status_id bigint, status character varying,services json, total numeric, subtotal numeric, customer_requested_amount numeric, discount_amount numeric, discount_percentage numeric, discount_total numeric, created_by bigint, customer_name character varying, created_date timestamp without time zone, preferred_date date,room_sqfts varchar,with_basement boolean) 
...
left join user_registration ur on ur.id = sb.assign_to and ur.is_active = true and ur.role_id = 3
left join user_registration ur2 on ur2.id = sb.created_by and ur2.is_active =true and ur2.role_id =4
WHERE (sb.id = tickt_id or -1=tickt_id)  and sb.is_active =true and (sb.status_id=p_status_id or -1=p_status_id) and 
(sb.created_by =custid or -1=custid) and (sb.assign_to =empid or -1=empid) 
group by sb.id, sb.booking_id, sb.slot_id, ms.slot_time, sb.hours, sb.add_on_hours, 
    sb.full_name, sb.phone, sb.email, sb.address, sb.assign_to, ur.first_name, ur.last_name,
    ur.email, sb.status_id, ms2.status, sb.total, sb.subtotal, 
    sb.customer_requested_amount, sb.discount_amount, sb.discount_percentage, 
